...
class CardTimeTableControllerUnitTests: XCTestCase {
  ...
  override func setUp() {
    ...
    if let tableView = cardTableViewControllerUnderTest.tableView {
      ...
      // By doing this, we cause the backgroundView to be initialized
      XCTAssertGreaterThan(numberOfRows, -1,
                           "Number of rows should never be less than 0." +
                            " This method is used to initialize backgroundView for future tests.")
  ...
  */
  ...
  func test_InitialTableView_OffsetY_IsNegative() {
    ...
    XCTAssertTrue(offset.y <= offsetPoint.y,
                  "Offset is incorrectly set." +
                  " Incorrect offset can cause refreshController's attributed title to show in empty list." +
                  " The y coordinate should be negative. The value of the offset point was:" +
  ...
  func test_NumberOfSections_IsZero_OnEmptyTable() {
    XCTAssertTrue(cardTableViewControllerUnderTest.items.isEmpty,
                  "Items should not be empty when testing if numberOfSections is 0 for empty table")
    XCTAssertTrue(numberOfSections == 0,
                  "The number of sections should be 0 on an empty item set. The value was: \(numberOfSections)")
  }
  ...
  func test_UpdateTable_ForEmptyJSON() {
    ...
    XCTAssertTrue(cardTableViewControllerUnderTest.noPrediction,
                  "noPrediction should be set to true." +
                  "Actual value was: \(cardTableViewControllerUnderTest.noPrediction)")
  }
  ...
  /**
   Asserts that `noPrediction` is false upon having more than one json item or an emtpy array.
   Also asserts that the json assigned to the view controller is the same as the on being passed to the update function.
   */
  func test_UpdateTable_For_OneJSONObject() {
    ...
    XCTAssertFalse(cardTableViewControllerUnderTest.noPrediction,
                   "noPrediction should be set to false if there is one item and it isn't \"No arrival times\" ")
    XCTAssertTrue(cardTableViewControllerUnderTest.items == json,
                  "JSON results should match. Actual data was \(cardTableViewControllerUnderTest.items)")
  }
  ...
  func test_NumberOfSections_GreaterThan_One_For_NoneEmptyItems() {
    ...
    XCTAssertTrue(numberOfSections == 1,
                  "Number of sections is not 1 when it should be. Actual result was \(numberOfSections)")
  }
  ...
  func test_NumberOfSections_IsZero_ForNoPrediction() {
    ...
    cardTableViewControllerUnderTest.updateTable(json)
    XCTAssertTrue(cardTableViewControllerUnderTest.noPrediction,
                  "No predictions should be set to true after passing in a no prediction string")

    XCTAssertTrue(numberOfSections == 1,
                  "There should only be one section after noPrediction is set to true." +
                  " The value was: \(numberOfSections)")
  }
  ...
  func test_NumberOfRows_Equals_Json_Length() {
    ...
    cardTableViewControllerUnderTest.updateTable(json)
    XCTAssertTrue(numberOfSections == 1,
                  "Number of sections is not one when it should be. The actual value was: \(numberOfSections)")

    XCTAssertTrue(numberOfRows == json.count,
                  "Number of rows did not equal the size of the json array." +
                  " The value for the row was: \(numberOfRows) \n The count of the json array was: \(json.count)")
  }
}
