...
class ThreeDTouchCoreDataManagerTests: XCTestCase {
  ...
  // NOTE: `shortcutItems` in `UIApplication.sharedApplication()` is not the same
  // when running tests as it is in your normal app. Hence, we don't have to mock here (though it would be nice)
  ...
  func test_Assert_updateShortcutItemsWithFavorites_For_Single_Favorite() {
    ...
    do {
      ...
      XCTAssertTrue(favorites.count > 0,
                    "There should be NSManagedObjects to work with, can't create shortcut items without them")
      ...
      XCTAssertTrue(favorite.stop == title,
                    "Stop should be the same as the title of the shortcut." +
                    "  Title was: \(title). Should have been: \(favorite.stop!)")
      XCTAssertTrue(favorite.route == subtitle,
                    "Route should be the same as the subtitle." +
                    " Subtitle was: \(subtitle). Should have been: \(favorite.route!)")
      XCTAssertTrue(type == shortcut?.type,
                    "Identifiers should match for given shortcut. See line: \(#line)")
    } catch {
  ...
  func test_Assert_updateShortcutItemsWithFavorites_For_Three_Favorites() {
    ...
    do {
      ...
      XCTAssertTrue(fetchedFavorites.count == favorites.count,
                    "There should be the same amount of favorites stored as there were fetched")
      ...
      for index in (0...2) {
        ...
        XCTAssertTrue(title == favorite.stop,
                      "Title should match stop number for favorite." +
          " Title was: \(title). Should have been: \(favorite.stop!)")
        XCTAssertTrue(subtitle == favorite.route,
                      "Subtitle should match route for favorite. Subtitle was \(subtitle)." +
          " Should have been: \(favorite.route!)")
        XCTAssertTrue(shortcutItems?[index].type == type,
                      "Identifiers should match for given shorcut. See line: \(#line)")
      }
    } catch {
